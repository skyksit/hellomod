...
const planetGen = extend(PlanetGenerator, {
  ...
  getBlock(position) {
    ...
    let tnoise = this.noise.octaveNoise3D(
      7,
      0.56,
      1 / 3,
      position.x,
    ...
    let res =
      arr[Mathf.clamp(Math.floor(temp * arr.length), 0, arr[0].length - 1)][
        Mathf.clamp(Math.floor(height * arr[0].length), 0, arr[0].length - 1)
      ];
    return res;
  },
  ...
  generate(tiles, sec) {
    ...
    this.tiles.each((x, y) => {
      gen.reset();
      let position = this.sector.rect.project(
        x / tiles.width,
        y / tiles.height
      );
      ...
      connect(to) {
        ...
        gend.brush(
          gend.pathfind(
            ...
            (tile) =>
              (tile.solid() ? 5 : 0) +
              gend.noiseOct(tile.x, tile.y, 1, 1, 1 / nscl) * 60,
            Astar.manhattan
          ),
          stroke
        );
    ...
    let spawn = null;
    let enemies = new Seq();
    let enemySpawns = rand.random(
      1,
      Math.max(Mathf.floor(this.sector.threat * 4), 1)
    );
    ...
    for (let i = 0; i < 360; i += angleStep) {
      ...
      if (waterTiles <= 4 || i + angleStep >= 360) {
        ...
        for (let j = 0; j < enemySpawns; j++) {
          ...
          Tmp.v1
            .set(cx - this.width / 2, cy - this.height / 2)
            .rotate(180 + enemyOffset)
    ...
    if (
      this.noise.octaveNoise3D(
        ...
        0.5,
        scl,
        this.sector.tile.v.x,
        this.sector.tile.v.y,
        this.sector.tile.v.z
      ) *
        nmag +
        poles >
      0.25 * addscl
    ...
    this.pass((x, y) => {
      ...
      for (let i = ores.size - 1; i >= 0; i--) {
        ...
        if (
          Math.abs(
            0.5 - this.noiseOct(offsetX, offsetY + i * 999, 2, 0.7, 40 + i * 2)
          ) >
            0.22 + i * 0.01 &&
          Math.abs(
            0.5 - this.noiseOct(offsetX, offsetY - i * 999, 1, 1, 30 + i * 4)
          ) >
            0.37 + freq
        ) {
          this.ore = entry;
    ...
    this.pass((x, y) => {
      ...
      if (
        ...
        this.floor != Blocks.mud &&
        this.floor.asFloor().hasSurface()
      ) {
    ...
    if (this.sector.hasEnemyBase()) {
      this.basegen.generate(
        tiles,
        enemies.map((r) => this.tiles.getn(r.x, r.y)),
        this.tiles.get(spawn.x, spawn.y),
      ...
      state.rules.winWave = this.sector.info.winWave =
        10 + 5 * Math.max(difficulty * 10, 1);
    }

    let waveTimeDec = 0.4;

    state.rules.waveSpacing = Mathf.lerp(
      60 * 65 * 2,
      60 * 60 * 1,
      Math.floor(Math.max(difficulty - waveTimeDec, 0) / 0.8)
    );
